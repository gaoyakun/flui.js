class TreapNode<T> {
    ...
    private static _randint () {
        return this._seed = (this._seed * 48271 % ((1 << 31) - 1));
    }
}
...
function bstGetRank<T> (node: BSTNode<T>, value: T, fnComp: (a:T, b:T)=>number): number {
    if (!node) {
        ...
        if (cmp < 0) {
            ...
            return bstGetRank (node.right, value, fnComp) + (node.left ? node.left.value.size : 0) + node.value.count;
        }
    }
}
...
export class Treap<T> {
    ...
    constructor (root?: BSTNode<T>, fnComp?: (a:T, b:T)=>number) {
        ...
        this._fnComp = fnComp || function(a: any, b: any) { return a - b; };
    }
    get root (): BSTNode<T> {
        return this._root;
    }
